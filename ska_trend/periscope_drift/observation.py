...
class Observation(astromon.observation.Observation):
    ...
    def get_events(self):
        ...
        outfile = self.workdir / events_file.name.replace(".fits", "_radec.fits")
        self.ciao(
            "dmcopy",
    ...
    def get_sources(self):
        ...
        pileup_img_file = list(self.file_glob("**/*_pileup.img"))
        if pileup_img_file:
        ...
        src["acis"] = self.is_acis
        src["hrc"] = self.is_hrc

        src["theta"] = [
            self.dmcoords("theta", option="cel", celfmt="deg", ra=ra, dec=dec)
        ...
        max_ang_corr = np.sqrt(src["max_yang_corr"] ** 2 + src["max_zang_corr"] ** 2)

        src = src[
            (
                (src["d_OOBAGRD3"] > 0.05)
                | (src["d_OOBAGRD6"] > 0.005)
                | (max_ang_corr > 0.1)
            )
            & (src["snr"] > 10)
            & (src["net_counts"] > 1000)
            # distance to closest source. Extended sources can be split into several sources
            & (src["near_neighbor_dist"] > 6)
    ...
    @Cache("periscope_drift_data", fmt="pickle", dir_attribute="cache_dir")
    def get_periscope_drift_data(self):
        src = self.get_sources()
        events = self.get_events()
        telem = fetch_telemetry(
            CxoTime(float(self.get_obspar()["tstart"])),
            CxoTime(float(self.get_obspar()["tstop"])),
        )
        ...
        info = {
            "obsid": self.obsid,
            "tstart": float(self.get_obspar()["tstart"]),
            "tstop": float(self.get_obspar()["tstop"]),
        ...
        binned_data = {
            source["id"]: dat
            for source in src
            if (dat := process_source(self, source, events, correction))
        }

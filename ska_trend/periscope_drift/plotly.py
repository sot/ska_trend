...
def get_drift_histograms(sources):
    ...
    r, g, b = webcolors.name_to_rgb("orange")
    histogram_difference = go.Histogram(
        {
            "name": "Actual-Expected Drift",
            "x": sources["drift_actual"] - sources["drift_expected"],
            "xbins": {
                "start": -3,
                "end": 3,
                "size": 0.1,
    ...
    return histogram_actual, histogram_expected, histogram_difference
...
def get_drift_figure(sources):
    ...
    histogram_actual, histogram_expected, histogram_difference = get_drift_histograms(
        sources
    )
    drift_scatter, identity = get_drift_scatter_objects(sources)
    ...
    fig.add_traces([drift_scatter, identity], rows=1, cols=1)
    fig.add_traces([histogram_actual, histogram_expected], rows=1, cols=2)
    fig.add_traces([histogram_difference], rows=1, cols=3)
    ...
    fig.update_xaxes(
        {"range": [0, 3], "title": "Intra-observation drift (arcsec)"},
        row=1,
        col=2,
    )
...
def get_chi2_figure(sources):
    ...
    fig.add_trace(histograms["chi2"]["zag"], row=1, col=1)
    fig.add_trace(histograms["p_value"]["yag"], row=1, col=2)
